...
> **Статус: контур доставки ещё не развёрнут** (внедряется Планом 2: GitHub,
> сервер, боевые команды). До этого команды ниже — заглушки, деплой выполняет
> только разработчик вручную, а prod-БД и `DATABASE_URL_PROD_RO` не существуют.
> Эту пометку удалим при вводе контура в строй.
