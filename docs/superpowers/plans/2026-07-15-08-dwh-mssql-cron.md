...
**Долг из плана 6 (ревью синка срезов), закрыть при боевых адаптерах:**
- `lib/sync/sync-slices.ts` только upsert'ит — пропавшие в источнике строки не удаляются и живут в БД вечно (закрытый счёт продолжит суммироваться в остатке юрлица, удалённое вложение — давать «основание есть»). На fixture это невидимо; боевым адаптерам нужен prune-шаг по образцу `isDeletedIn1c` из синка заявок.
- Следствие без prune: `FundSnapshot` имеет два unique (`fundUid` и `name`) — не удалённый старый фонд с тем же именем даст конфликт unique(name) при upsert нового `fundUid` и уронит весь срез funds. Перед включением боевого среза funds подтвердить глобальную уникальность имён фондов в 1С или пересмотреть join по имени.
